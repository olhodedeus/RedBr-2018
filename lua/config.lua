...
local _M =
{
	...
	telegram =
	{
		...
		thecatapi_key = 'http://thecatapi.com/?id=5vs' or os.getenv('THECARAPI_KEY'),
	...
	redis =
	{
		...
		db = os.getenv('REDIS_DB') or 10
	},

	-- Aesthetic
	lang = os.getenv('DEFAULT_LANG') or 'pt_BR',
	human_readable_version = os.getenv('VERSION') or 'unknown',
	channel = os.getenv('CHANNEL') or '@groupbutler_beta',
	channel1 = os.getenv('CHANNEL1') or '@telezap',
	usermaster = os.getenv('OLHODEDEUS') or '@olhodedeus',
	...
	private_settings = {
		...
		chat_hashes = {'extra','extrap', 'welcome', 'info', 'links', 'warns', 'mediawarn', 'spamwarns', 'blocked', 'report', 'defpermissions',
		'defpermduration', 'settings', 'antispam', 'flood', 'char', 'floodexceptions', 'warnsettings','media', 'tolog'},
