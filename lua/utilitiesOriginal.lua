...
-- Extrap
function utilities.getExtrapList(chat_id)
...
function utilities.changeSettingStatus(chat_id, field)
	local turned_off = {
		...
		extrap = i18n("/extra commands are now available only for moderator"),
	...
	local turned_on = {
		...
		extrap = i18n("/extra commands are now available for all"),
...
-- fim
function utilities.getAbout(chat_id)
	local hash = 'chat:'..chat_id..':about'
	local about = db:get(hash)
    if not about then
    	local message = '*SEM DESCRIÇÃO* para este grupo.'
