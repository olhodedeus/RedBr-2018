...
local function is_locked(chat_id)
		...
		local current = db:hget(hash, 'Extrap')
		if current == 'off' then
			return true
		else
			return false
		...
		if not blocks[3] and not msg.reply then return end
			...
			if msg.reply and not blocks[3] then
				...
				if not file_id then
					...
					db:hset('chat:'..msg.chat.id..':extrap', blocks[2], to_save)
					api.sendReply(msg, i18n("This media has been saved as a response to %s"):format(blocks[2]))
				end
		else
				local hash = 'chat:'..msg.chat.id..':extrap'
				local new_extrap = blocks[3]
				local reply_markup, test_text = u.reply_markup_from_text(new_extrap)
				...
				if not res then
					...
					db:hset(hash, blocks[2]:lower(), new_extrap)
					local msg_id = res.result.message_id
				api.editMessageText(msg.chat.id, msg_id, i18n("Command '%s' saved!"):format(blocks[2]))
				end
			end
	elseif blocks[1] == 'extrap list' then
		local text = u.getExtrapList(msg.chat.id)
			if not msg.from.admin and not is_locked(msg.chat.id) then
			api.sendMessage(msg.from.id, text)
		else
			api.sendReply(msg, text)
		end
		elseif blocks[1] == 'extrap del' then
				if not msg.from.admin then return end
			local deleted, not_found, found = {}, {}
			local hash = 'chat:'..msg.chat.id..':extrap'
			for extrap in blocks[2]:gmatch('(/[%w_]+)') do
				found = db:hdel(hash, extrap)
				if found == 1 then
					deleted[#deleted + 1] = extrap
				else
					not_found[#not_found + 1] = extrap
				end
			end
		...
		local extrap = blocks[1] == 'start' and '/'..blocks[3] or blocks[1]
		--print(chat_id, extra)
			local hash = 'chat:'..chat_id..':extrap'

			local text = db:hget(hash, extrap:lower()) or db:hget(hash, extrap)
			...
			local link_preview = text:find('telegra%.ph/') ~= nil
				...
				local res = true
				local code
				if msg.chat.id > tostring(0) or (is_locked(msg.chat.id) and not msg.from.admin) then --send it in private
					if not file_id then
			...
			if not res and code == 403.0 and msg.chat.id < tostring(0) and not u.is_silentmode_on(msg.chat.id) then
				--if the user haven't started the bot and silent mode is off
					api.sendReply(msg, i18n("_Please_ [start me](%s) _so I can send you the answer_")
						:format(u.deeplink_constructor(msg.chat.id, extrap:sub(2, -1))), true)
				end
		end
end
...
plugin.triggers = {
	onTextMessage = {
		config.cmd..'(extrap)$',
		config.cmd..'(extrap) (/[%w_]*) (.*)$',
		config.cmd..'(extrap) (/[%w_]*)',
		config.cmd..'(extrap del) (.+)$',
		config.cmd..'(extrap list)$',
		'^/(start) (-?%d+)_([%w_]+)$',
		'^(/[%w_]+)$'
	}
}
