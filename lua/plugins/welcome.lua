...
local function get_welcome(msg)
	...
	elseif type == 'custom' then
		...
		local reply_markup =
			{
				inline_keyboard={
				{{text = i18n("➕ Dicas Aqui"), url = 'https://telegram.me/EncontreAqui'}},
				{{text = i18n("🌐 Nosso Site"), url = 'http://portaltelezap.com.br/'}},
				{{text = i18n('Read the rules'), url = u.deeplink_constructor(msg.chat.id, 'rules')}}
			
			}
			}
		return i18n("Seja bem-vindo(a) ao Grupo %s\nLeia as regras."):format(msg.new_chat_member.first_name:escape()),reply_markup
	end
end

function plugin.onTextMessage(msg, blocks)
	if blocks[1] == 'welcome' or blocks[1] == 'bemvindo' or blocks[1] == 'welcome@redlolbot' or blocks[1] == 'bemvindo@redlolbot' then
	...
	if blocks[1] == 'new_chat_member' then
		...
		if is_blocked(msg.chat.id, msg.new_chat_member.id) and not msg.from.mod then
			local res = api.banUserb(msg.chat.id, msg.new_chat_member.id)
			if res then
				unblockUser(msg.chat.id, msg.new_chat_member.id)
				local name = u.getname_final(msg.new_chat_member)
				api.sendMessage(msg.chat.id, i18n("%s banned: the user was blocked"):format(name), 'html')
...
plugin.triggers = {
	onTextMessage = {
		config.cmd..'(welcome) (.*)$',
		config.cmd..'(welcome@redlolbot) (.*)$',
		config.cmd..'(bemvindo) (.*)$',
		config.cmd..'(bemvindo@redlolbot) (.*)$',
		config.cmd..'set(welcome) (.*)$',
		config.cmd..'set(welcome@redlolbot) (.*)$',
		config.cmd..'set(bemvindo) (.*)$',
		config.cmd..'(welcome)$',
		config.cmd..'(welcome@redlolbot)$',
		config.cmd..'(bemvindo)$',
		config.cmd..'(bemvindo@redlolbot)$',
		...
		config.cmd..'set(welcome@redlolbot)$',
		config.cmd..'set(bemvindo)$',
		config.cmd..'set(bemvindo@redlolbot)$',
		config.cmd..'(goodbye) (.*)$',
