	...
	if msg.from.admin then
		if blocks[1] == 'snap' then
			...
			if last_user then
				...
				local text = i18n(
					[[<i>I'm sorry, this command has been used for the last time less then 3 hours ago by </i> %s (ask him for the file). Wait [<code>%s</code>] to use it again.
				...
				api.sendDocument2(msg.from.id, file_path, nill, ('#snap\n%s'):format(msg.chat.title))
			end
		end
