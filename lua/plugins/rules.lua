...
function plugin.onTextMessage(msg, blocks)
	...
	if blocks[1] == 'rules' or blocks[1] == 'start' or blocks[1] == 'regras' or blocks[1] == 'rules@redlolbot' or blocks[1] == 'regras@redlolbot' then
	...
	if blocks[1] == 'setrules' or blocks[1] == 'setregras' or blocks[1] == 'setrules@redlolbot' or blocks[1] == 'setregras@redlolbot'then
...
plugin.triggers = {
	onTextMessage = {
		...
		config.cmd..'(rules)$',
		config.cmd..'(regras)$',
		config.cmd..'(setrules@redlolbot)$',
		config.cmd..'(setrules@redlolbot) (.*)',
		config.cmd..'(setregras@redlolbot)$',
		config.cmd..'(setregras@redlolbot) (.*)',
		config.cmd..'(rules@redlolbot)$',
		config.cmd..'(regras@redlolbot)$',
		'^/(start) (-?%d+)_rules$'
	}
}
