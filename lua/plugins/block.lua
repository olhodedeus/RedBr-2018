...
function plugin.onTextMessage(msg, blocks)
    if msg.chat.id < tostring(0) and u.is_allowed('hammer', msg.chat.id, msg.from) then
        ...
        if blocks[1] == 'block' or blocks[1] == 'unblock' then
            if not blocks[2] and msg.reply then
                if not msg.reply.forward_from then
                    text = i18n("<i>Please reply to a forwarded message to block the original sender, or provide me a list of usernames</i>")
                else
                    if u.is_mod(msg.chat.id, msg.reply.forward_from.id) and blocks[1] == 'block'  then
                        ...
                        text = i18n("<i>You can't block an admin or a mod</i>")
                    else
                        if blocks[1] == 'block' then
                            local is_new = block.User(msg.chat.id, msg.reply.forward_from)
                            if not is_new then
                                text = i18n("<i>This user is already in the blocked list</i>")
                            else
                                local name = u.getname_final(msg.reply.forward_from)
                                text = i18n("%s added to the blocked list"):format(name)
                        ...
                        elseif blocks[1] == 'unblock' then
                            ...
                            if not is_not_blocked then
                                text = i18n("<i>This user is not in the blocked list</i>")
                            else
                                local name = u.getname_final(msg.reply.forward_from)
                                text = i18n("%s removed from the blocked list"):format(name)
        ...
        if blocks[1] == 'blockedlist' or blocks[1] == 'blocklist' then
            text = i18n('Where do you want to receive the list?')
            reply_markup = {inline_keyboard={{
                {text = i18n("Here"), callback_data = 'blockedlist:group'},
                {text = i18n("Private"), callback_data = 'blockedlist:private'}
            }}}
        end
...
function plugin.onCallbackQuery(msg, blocks)
    if u.is_allowed('info', msg.chat.id, msg.from) then
        ...
        if not next(users_blocked) then
            api.editMessageText(msg.chat.id, msg.message_id, i18n('<i>The list is empty</i>'), 'html')
        else
            local text = i18n([[<b>List of blocked users (%d)</b>:
...
<i>When a blocked user joins and gets banned, he's removed from this list</i>]]):format(#users_blocked, table.concat(users_blocked, '\n- '))
            ...
            if blocks[1] == 'private' then
                ...
                if res then
                    api.editMessageText(msg.chat.id, msg.message_id, i18n("I've sent you the list in private"))
                end
            ...
            if not res then --something went wrong while sending/editing the message
                local text
                if code == 118 then
                    text = i18n("<i>I'm sorry, the list is too long. I can't show you the whole list now. This problem will be solved soon</i>")
                elseif code == 144 or code == 145 then
                    text = i18n("<i>Something went wrong with the formattation of the list</i>")
                elseif (code == 403 or code == 110) and blocks[1] == 'private' then
                    api.answerCallbackQuery(msg.cb_id, i18n("You need to start me first.\nClick/tap here: t.me/%s"):format(bot.username), true)
                    return
                else
                    text = i18n("<i>An unknown error occurred</i>. Code: %d"):format(code)
                end
                api.editMessageText(msg.chat.id, msg.message_id, text, 'html')
            end
        end
    else
        api.answerCallbackQuery(msg.cb_id, i18n("You are not allowed to use this button"), true)
    end
end
