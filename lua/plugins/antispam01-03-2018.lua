			...
			if not whitelisted then
				...
				if warns_received >= max_allowed then
					...
					elseif status == 'del' then
						--just delete
						api.deleteMessage(msg.chat.id, msg.message_id)
					elseif status == 'avs' then
						api.deleteMessage(msg.chat.id, msg.message_id)
						db:hdel('chat:'..msg.chat.id..':spamwarns', msg.from.id)
						--just delete
						api.deleteMessage(msg.chat.id, msg.message_id)
					elseif status ~= 'del' then
...
local function toggleAntispamSetting(chat_id, key)
	...
	local next_state = { ['alwd'] = 'warn', ['warn'] = 'del', ['del'] = 'avs', ['avs'] = 'alwd' }
...
local function changeWarnsNumber(chat_id, action)
	...
	elseif current == 15 and action == 'raise' then
...
local function doKeyboard_antispam(chat_id)
	...
	for field, _ in pairs(config.chat_settings['antispam']) do
		if field == 'links' or field == 'linkswp' or field == 'antipo' or field == 'antico' or field == 'forwards' then
			local icon = '✅'
	...
	local line = {
		{text = i18n('Warns: ')..warns, callback_data = 'antispam:alert:warns:'..locale.language},
...
• ✅🗑 = *Allowed and Delete* (erases but does not apply warning)
...
When set on `delete`, the bot doesn't warn users until they are about to be kicked/banned/muted (at the second-to-last warning)
			...
			if blocks[1] == 'toggle' then
				if blocks[2] == 'forwards' or blocks[2] == 'links' or blocks[2] == 'antipo' or blocks[2] == 'antico' or blocks[2] == 'linkswp' then
