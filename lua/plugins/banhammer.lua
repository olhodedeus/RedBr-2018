...
local function markup_tempban(chat_id, user_id, time_value)
	...
	local markup = {inline_keyboard={
		...
		{--second line
			{text = i18n('minutes'), callback_data = ('tempban:ban:m:%s:%s'):format(user_id, chat_id)},
			{text = i18n('hours'), callback_data = ('tempban:ban:h:%s:%s'):format(user_id, chat_id)},
			{text = i18n('days'), callback_data = ('tempban:ban:d:%s:%s'):format(user_id, chat_id)},
		}
	}}

	return markup
end
local function markup_tempmute(chat_id, user_id, time_value)
	local key = ('chat:%s:%s:tmutevalue'):format(chat_id, user_id)
	time_value = time_value or (db:get(key) or 3)

	local markup = {inline_keyboard={
		{--first line
			{text = '➖', callback_data = ('tempmute:val:m:%s:%s'):format(user_id, chat_id)},
			{text = '🕑 '..time_value, callback_data = 'tempmute:nil'},
			{text = '➕', callback_data = ('tempmute:val:p:%s:%s'):format(user_id, chat_id)}
		},
		{--second line
			{text = i18n('minutes'), callback_data = ('tempmute:mute:m:%s:%s'):format(user_id, chat_id)},
			{text = i18n('hours'), callback_data = ('tempmute:mute:h:%s:%s'):format(user_id, chat_id)},
			{text = i18n('days'), callback_data = ('tempmute:mute:d:%s:%s'):format(user_id, chat_id)},
		}
	}}
...
local function get_motivation(msg)
	if msg.reply then
		return msg.text:match(("%sban (.+)"):format(config.cmd))
			...
			or msg.text:match(("%smute (.+)"):format(config.cmd))
			or msg.text:match(("%sdesmute (.+)"):format(config.cmd))
			or msg.text:match(("%stempban .+\n(.+)"):format(config.cmd))
			or msg.text:match(("%stempmute .+\n(.+)"):format(config.cmd))
	else
		if msg.text:find(config.cmd.."ban @%w[%w_]+ ") or msg.text:find(config.cmd.."mute @%w[%w_]+ ") or msg.text:find(config.cmd.."kick @%w[%w_]+ ") then
			return msg.text:match(config.cmd.."ban @%w[%w_]+ (.+)") or msg.text:match(config.cmd.."mute @%w[%w_]+ (.+)") or msg.text:match(config.cmd.."kick @%w[%w_]+ (.+)")
		elseif msg.text:find(config.cmd.."ban %d+ ") or msg.text:find(config.cmd.."mute %d+ ") or msg.text:find(config.cmd.."kick %d+ ") then
			return msg.text:match(config.cmd.."ban %d+ (.+)") or msg.text:match(config.cmd.."mute %d+ (.+)") or msg.text:match(config.cmd.."kick %d+ (.+)")
		elseif msg.entities then
			return msg.text:match(config.cmd.."ban .+\n(.+)") or msg.text:match(config.cmd.."mute .+\n(.+)") or msg.text:match(config.cmd.."kick .+\n(.+)")
		end
	end
end
...
function plugin.onTextMessage(msg, blocks)
	if msg.chat.type ~= 'private' then
		if u.can(msg.chat.id, msg.from.id, "can_restrict_members") then
			...
			if blocks[1] == 'tempmute' then
				local time_value = msg.text:match(("%stempmute.*\n"):format(config.cmd).."(%d+)")
				if time_value then --save the time value passed by the user
					if tonumber(time_value) > 100 then
						time_value = 100
					end
					local key = ('chat:%s:%s:tmutevalue'):format(msg.chat.id, user_id)
					db:setex(key, 3600, time_value)
				end

				local markup = markup_tempmute(msg.chat.id, user_id)
				api.sendReply(msg, i18n('Use -/+ to edit the value, then select a timeframe to temporary mute the user'), nil,
					markup)
			end
			if blocks[1] == 'kick' or blocks[1] == 'remover' then
			...
			if blocks[1] == 'ban' or blocks[1] == 'banir' then
			...
			if blocks[1] == 'mute' or blocks[1] == 'silenciar' or blocks[1] == 'mudo' then
				local res, _, motivation = api.muteUser(chat_id, user_id)
				if not res then
					if not motivation then
						motivation = i18n("I can't kick this user.\n"
								.. "Either I'm not an admin, or the targeted user is!")
					...
					u.logEvent('mute', msg, {motivation = get_motivation(msg), admin = admin, user = kicked, user_id = user_id})
					api.sendMessage(msg.chat.id, i18n("%s muted %s!"):format(admin, kicked), 'html')
				end
			end
			if blocks[1] == 'desmute' or blocks[1] == 'unmute' or blocks[1] == 'livre' then
			local res, _, motivation = api.desmuteUser(chat_id, user_id)
				if not res then
					if not motivation then
						motivation = i18n("I can't kick this user.\n"
								.. "Either I'm not an admin, or the targeted user is!")
					...
					u.logEvent('mute', msg, {motivation = get_motivation(msg), admin = admin, user = kicked, user_id = user_id})
					api.sendMessage(msg.chat.id, i18n("%s removed the restrictions %s!"):format(admin, kicked), 'html')
				end
			end
			...
			if blocks[1] == 'unban' or blocks[1] == 'desbanir' then
	...
	if not u.can(msg.chat.id, msg.from.id, 'can_restrict_members') then
		...
		elseif matches[1] == 'val' then
			...
			elseif matches[2] == 'p' then
				...
				local markup = markup_tempmute(msg.chat.id, user_id, new_value)
		...
		elseif matches[1] == 'mute' then
			...
			local res, _, motivation = api.tempmuteUser(msg.chat.id, user_id, until_date)
			if not res then
				motivation = motivation or i18n("I can't kick this user.\n"
					.. "I am not allowed to ban or the target user is an admin")
				api.editMessageText(msg.chat.id, msg.message_id, motivation)
			else
				local text = i18n("User muted for %d %s"):format(time_value, timeframe_string)
...
plugin.triggers = {
	...
	onCallbackQuery = {
		...
		'^###cb:tempban:(nil)$',
		'^###cb:tempmute:(val):(%a):(%d+):(-%d+)',
		'^###cb:tempmute:(mute):(%a):(%d+):(-%d+)',
		'^###cb:tempmute:(nil)$'
	}
}
