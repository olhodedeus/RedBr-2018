...
local humanizations = {
	...
	['linkswp'] = i18n('links whatsapp'),
	['antipo'] = i18n('Anti Po'),
	['antico'] = i18n('Anti commands'),
	['anticsujos'] = i18n('Anti Dirty Content'),
	...
	if not msg.inline and msg.spam and msg.chat.id < 0 and not msg.cb and not msg.from.admin then
		...
		if status and status ~= 'alwd' then
			...
			--		end
					...
					elseif action == 'avs' then
						res = api.sendMessageHTML(msg.chat.id, 'devagar ai fera '.. msg.from.first_name..'.', true)
					end
					if res then
						...
						local action = (db:hget('chat:'..msg.chat.id..':antispam', 'action')) or config.chat_settings['antispam']['action']
						if status == 'del' and action ~= 'avs' and warns_received == max_allowed - 1 then
				...
				local name_pretty = {links = i18n("telegram.me link"), forwards = i18n("message from a channel"), linkswp = i18n("links whatsapp")}
...
local function toggleAntispamSetting(chat_id, key)
	...
	elseif key == 'antico' then
	if new == 'alwd' then
			return i18n("commands are allowed")
		elseif new == 'warn' then
			return i18n("warn for commands")
		elseif new == 'del' then
			return i18n("commands will be deleted")
		elseif new == 'avs' then
			return i18n("Allowed and deleted")
		end
	elseif key == 'anticsujos' then
	if new == 'alwd' then
			return i18n("dirty content allowed")
		elseif new == 'warn' then
			return i18n("warn for dirty content")
		elseif new == 'del' then
			return i18n("dirty content will be deleted")
		elseif new == 'avs' then
			return i18n("Allowed and deleted")
		end
	elseif key == 'antipo' then
	if new == 'alwd' then
			return i18n("offenses are allowed")
		elseif new == 'warn' then
			return i18n("offenses will be warned")
		elseif new == 'del' then
			return i18n("offenses will be deleted")
		end
	end
end
...
local function changeAction(chat_id)
	...
	elseif current == 'mute' then new_action = 'avs'
	elseif current == 'avs' then new_action = 'ban' end
...
local function get_alert_text(key)
	...
	elseif key == 'linkswp' then
		return i18n("Allow/forbid whatsapp links")
	elseif key == 'antipo' then
		return i18n("Allow/forbid palavras de baixo calao")
	elseif key == 'antico' then
		return i18n("Allow/forbid sent of commands /")
	elseif key == 'anticsujos' then
		return i18n("Allow/forbid sent of dirty content")
...
local function doKeyboard_antispam(chat_id)
	...
	for field, _ in pairs(config.chat_settings['antispam']) do
		if field == 'links' or field == 'linkswp' or field == 'antipo' or field == 'antico' or field == 'anticsujos' or field == 'forwards' then
			local icon = '✅'
			local status = (db:hget('chat:'..chat_id..':antispam', field)) or config.chat_settings['antispam'][field]
			if status == 'warn' then
				icon = '❌'
			elseif status == 'del' then icon = '🗑'
			elseif status == 'avs' then icon = '✅🗑' end
			local line = {
	...
	--back button
	table.insert(keyboard.inline_keyboard, {{text = action, callback_data = 'antispam:toggle:action:'..chat_id}})
...
When set on `delete`, the bot doesn't warn users until they are about to be kicked/banned/muted (at the second-to-last warning)
			...
			if blocks[1] == 'toggle' then
				if blocks[2] == 'forwards' or blocks[2] == 'links' or blocks[2] == 'antipo' or blocks[2] == 'antico' or blocks[2] == 'anticsujos' or blocks[2] == 'linkswp' then
