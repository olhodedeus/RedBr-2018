...
[📢 Rede Telezap 🚀](https://telegram.me/GrupoTelezap) - Grupo de Divulgações]])

elseif key == 'redlol' then
		return i18n([[
*Comandos do RED.*

`/sorteio`: Para iniciar um sorteio envie `/sorteio [descrição]` e `/sossorteio`: para obter a lista de comandos relacionado ao sorteio.

`/evento`: Para iniciar um evento, envie `/evento [descrição]` e `/ajudaevento`:para obter a lista de comandos relacionado ao sorteio.

`/lista`: Para receber uma lista de grupos, de vários gêneros.

`/mark`: Envie ao seu grupo `/mark [descrição]`. E o bot lhe retornará a mensagem. (Aceita Markdown).

`/markp`: Envie ao seu grupo `/markp` descrição. E o bot lhe retornará a mensagem. (Aceita Markdown com pré-visualização de links).

`/gatos`:  você receberá uma imagem aleatória de gatinhos.

`/ranking`: O ao enviar `/ranking` no seu grupo, o redlol enviar uma lista com as pessoas que mais falaram no grupo.

`/zerar`: utilize o para limpar o ranking, somente o dono pode.]])
...
local function do_keyboard_private()
	...
	keyboard.inline_keyboard = {
		{
			{text = i18n("➕ Add me to Groups"), url = 'https://telegram.me/RedbrBot?startgroup=new'}
		},
		{
...
local function dk_main()
	...
	keyboard.inline_keyboard = {
		...
		{{text = i18n('Groups'), callback_data = 'help:grupos'},
		{text = i18n('Redlol'), callback_data = 'help:redlol'}},
	}
...
function plugin.onCallbackQuery(msg, blocks)
	...
	elseif query == 'grupos' then
     text = get_helped_string('grupos')
     answerCallbackQuery_text = i18n('Grupos')
  elseif query == 'redlol' then
     text = get_helped_string('redlol')
